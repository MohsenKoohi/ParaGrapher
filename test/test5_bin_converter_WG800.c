/*
This program reads a Big WebGraph graph asynchronously (PARAGRAPHER_CSX_WG_800_AP) 
whose |V| < 2 ^ 32 and decompresses it with changing 8 Bytes ID of the endpoints to 4 Bytes
*/
...
void callback(paragrapher_read_request* req, paragrapher_edge_block* eb, void* in_offsets, void* in_edges, void* buffer_id, void* args)
{
	...
	// Changing 8 Bytes vertex IDs of endpoints to 4 Bytes
	{
		unsigned int* ui_edges = (unsigned int*)in_edges;
		for(unsigned long e = 0; e < ec; e++)
			ui_edges[e]= ui_edges[2 * e];
	}
...
{	
	...
	sprintf(output_file, "/dev/shm/test4.bin");
	...
	paragrapher_graph* graph = paragrapher_open_graph(args[1], PARAGRAPHER_CSX_WG_800_AP, NULL, 0);
	assert(graph != NULL);
	...
	{
		printf("\nThis program uses 4 Bytes for vertex IDs and cannot be run for this graph.\n");
		exit(-1);
	}
